...
public class PlayerRadarPulseTrigger : MonoBehaviour
{
    ...
    [Header("펄스 속성 설정")]
    [Tooltip("레이더가 커지고 투명해지면서 사라지는 총 시간")]
    ...
    public class ChainedPulseSpecificSettings
    {
        ...
        [SerializeField] public float overrideFadeInDuration = -1f; // 이제 이 값은 전체 애니메이션 시간을 제어합니다.
        [Tooltip("이 태그에 연쇄되는 펄스의 유지 시간")]
        [SerializeField] public float overrideHoldDuration = -1f; // 이 값은 더 이상 사용되지 않지만, 기존 구조 유지를 위해 남겨둠
        [Tooltip("이 태그에 연쇄되는 펄스의 페이드 아웃 시간")]
        [SerializeField] public float overrideFadeOutDuration = -1f; // 이 값은 더 이상 사용되지 않지만, 기존 구조 유지를 위해 남겨둠
    ...
    IEnumerator PulseEffectRoutine()
    {
        ...
        while (time < totalAnimationDuration)
        {
            ...
            if (_instanceMaterial.HasProperty(SHADER_PROP_FADETIME_ID))
            {
                // 스케일이 커지면서 알파값이 0으로 가도록 _FadeTime을 1.0f - t로 설정
        ...
        if (_instanceMaterial.HasProperty(SHADER_PROP_PULSE_COLOR_ID))
        {
            // _PulseColor도 초기 상태(완전히 투명)로 되돌림
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (isInPropagationList)
        {
            ...
            if (newChainedTrigger != null)
            {
                ...
                float currentFadeInDuration = (foundSettings != null && foundSettings.overrideFadeInDuration >= 0) ? foundSettings.overrideFadeInDuration : fadeOutDuration; // 기존의 fadeOutDuration을 연쇄 펄스의 전체 애니메이션 시간으로 사용
                float currentHoldDuration = 0f; // 더 이상 사용되지 않으므로 0으로 고정
                float currentFadeOutDuration = 0f; // 더 이상 사용되지 않으므로 0으로 고정
