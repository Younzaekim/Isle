...
public class RuneDecalController : MonoBehaviour
{
    ...
    private bool revealOnApproach = false;
    ...
    void Awake()
    {
        ...
        if (Application.isPlaying)
        {
            ...
            _runtimeDecalMaterialInstance.name = decalMaterial.name + "(RuntimeClone)";
            ...
            _isRevealed = initialAlpha > 0.01f;
    ...
    private void InitializeDecalInEditMode()
    {
        if (decalProjector == null || decalMaterial == null)
        {
            Debug.LogWarning($"[{gameObject.name}] InitializeDecalInEditMode:Decal Projector or Material이 할당되지 않음");
            return;
        }
    ...
    public void SetMaterialAlpha(float alpha)
    {
        ...
        {
            Debug.LogWarning($"[{gameObject.name}] SetMaterialAlpha:런타임 마테리얼이 Null임. 알파값 조절 못함");
        }
    }
    ...
    void OnDestroy()
    {
        ...
        if (_runtimeDecalMaterialInstance != null)
        {
            ...
            _runtimeDecalMaterialInstance = null;
        }
    }

    void OnEnable() //안전용도
    {
        if (Application.isPlaying && !_isRevealed) // 플레이 모드이고, 아직 드러나지 않은 상태라면
        {
            SetMaterialAlpha(initialAlpha); // initialAlpha 값으로 다시 설정 시도
        }
    }
